...
require "arel/qualifying_methods"
require "arel/table"
...
module Arel
  ...
  class << self
    ...
    # Build an unbound Arel expression. Also aliased as +Arel.[]+ for convenience.
    #
    #   Arel[:name]            # => Arel::Nodes::Identifier
    #   Arel[:name].eq("DHH")  # => Arel::Nodes::Equality
    #
    # An Identifier is a tableless column reference; it renders without a table
    # qualifier:
    #
    #   Arel[:name].eq("DHH").to_sql # => "name" = 'DHH'
    #
    ...
    #
    # Today only Symbol is accepted. Support for boolean and other kinds of
    # expressions will be added later.
    def expr(arg)
      case arg
      when Symbol
        identifier(arg)
      else
        raise ArgumentError, "Arel.expr does not support #{arg.class}"
      end
    end
    alias_method :[], :expr

    # Build an Arel::Nodes::Identifier — a tableless column reference.
    #
    #   Arel.identifier(:name)  # => Arel::Nodes::Identifier
    def identifier(name)
      Arel::Nodes::Identifier.new(name)
    end

    # Build an Arel::Nodes::QualifiedIdentifier — a column qualified by a table
    # (or a table qualified by a schema, etc).
    #
    #   Arel.qualify(:users, :name)        # "users"."name"
    #   Arel.qualify(:schema, :users)      # "schema"."users"
    #   Arel.qualify(:users, :name).qualify(:schema)
    #   # => "schema"."users"."name"
    def qualify(qualifier, column)
      Arel::Nodes::QualifiedIdentifier.new(qualifier, column)
    end
